...
+            x, y = event.pos().x(), event.pos().y()
+            for camera in self.cameras:
+                if camera.collidepoint(x, y):
+                    self.clicked_object = camera
+                    self.offset = (x - camera.x, y - camera.y)
+                    break
+            else:
+                self.clicked_object = None

+    def wheelEvent(self, event):
+        if self.clicked_object and isinstance(self.clicked_object, pygame.Rect):
+            delta = event.angleDelta().y()
+            if delta > 0:
+                self.clicked_object.inflate_ip(-20, -15)
+            elif delta < 0:
+                self.clicked_object.inflate_ip(20, 15)
+            self.update()
